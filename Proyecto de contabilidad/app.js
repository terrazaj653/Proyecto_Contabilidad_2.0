...
// Clean up old demo entries key from localStorage if present
localStorage.removeItem("erp_entries");

// Load transaction entries (partidas) from localStorage
let entries = JSON.parse(localStorage.getItem("erp_entries_prod"));

// Initialize with empty array if no entries exist
if (!entries) {
    entries = [];
    localStorage.setItem("erp_entries_prod", JSON.stringify(entries));
}
